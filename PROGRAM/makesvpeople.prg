PROCEDURE MakeSvPeople
 ...
 IF m.lIsNeedperiod
  MESSAGEBOX(CHR(13)+CHR(10)+'ÎÁÐÀÁÎÒÊÀ ÏÐÅÄÛÄÓÙÅÃÎ ÏÅÐÈÎÄÀ ÂÊËÞ×ÅÍÀ!'+CHR(13)+CHR(10),0+64,'')
 ENDIF 
 ...
 SCAN 
  ...
  IF EMPTY(bname) AND m.lIsNeedperiod
   IF SEEK(m.lpuid, 'lcais') AND !EMPTY(lcais.bname)
    ...
    IF MESSAGEBOX(CHR(13)+CHR(10)+'ËÏÓ ÍÅ ÏÐÅÄÑÒÀÂÈË ÎÒ×ÅÒ Â ÒÅÊÓÙÅÌ ÌÅÑßÖÅ,'+;
     CHR(13)+CHR(10)+'ÍÎ ÏÐÅÄÑÒÀÂÈË Â ÏÐÅÄÛÄÓÙÅÌ.'+;
     CHR(13)+CHR(10)+'ÏÅÐÅÍÅÑÒÈ ÝÒÓ ÈÍÔÎÐÌÀÖÈÞ Â ÒÅÊÓÙÈÉ ÌÅÑßÖ?'+CHR(13)+CHR(10),32+4, '')=6
...
*      IF date_in<=svppl.date_in && Ïîìåíÿíî ñ òî÷íîñòüþ äî íàîáîðîò 06.03.2014 ïî ïðîñüáå Ñàøè Áåññîíîâà!
...
*        SET STEP ON 
     ...
     CASE spos = '2' AND svppl.spos = '2' && Îáà ïî çàÿâëåíèþ
      IF date_in<=svppl.date_in
